...
>
> 更新：v3.0（2026-06-24）— 从"原型生成规范"改为"前端代码修改规范"
...
| **组件复用优先** | 优先使用前端代码中已有的 Element Plus 组件和共享组件（SearchForm、SearchItem、Table 等），不自行编写替代组件 |
| **样式忠实复制** | 从 Hub 获取 Vue 源码后，原样复制组件结构、CSS 类名、scoped styles，不做样式发挥 |
| **设计令牌对齐** | 颜色使用前端代码中的 CSS 变量或 Element Plus 主题变量，不硬编码自定义色值 |
...
| 维度 | 前端实际技术栈 | 修改方式 |
|------|--------------|---------|
| 框架 | Vue 3（Composition API） | 直接修改 .vue 文件 |
| UI 库 | Element Plus | 直接使用项目已安装的 Element Plus |
| 样式 | Element Plus 主题 + scoped CSS + Tailwind CSS | 直接修改 `<style scoped>` |
| 主色 | `#E63935`（红色系） | 使用项目已配置的主题变量 |
| 共享组件 | SearchForm / SearchItem / Table | 直接 import 使用，无需还原 |

> **v3.0 变化**：不再需要 CDN 引入 Vue/Element Plus/Tailwind。直接在前端仓库中修改，使用项目已安装的依赖。
...
| `el-descriptions` | 直接使用 Element Plus 组件 | 保持原有 :column 属性 |

> **v3.0 变化**：不再需要把共享组件"还原"为底层 Element Plus 结构。直接在前端项目中使用已封装的共享组件。
...
### 1.2 语义色映射（Element Plus 对齐）

| 场景 | 色值 | Element Plus 变量 | CSS 变量 |
|------|------|------------------|---------|
...
- ✅ 使用 Element Plus 组件（el-table / el-form / el-descriptions 等），不自定义替代组件
- ✅ 共享组件直接 import 使用（SearchForm / SearchItem / Table），不还原为底层结构
- ✅ 颜色使用 CSS 变量或 Element Plus 主题变量，不硬编码
...
- ❌ 自定义设计体系（不使用 Element Plus，自己写 CSS 组件库）
- ❌ 用 Bootstrap、Ant Design 等其他 UI 框架替代 Element Plus
...
| 组件类型一致 | 新增元素与源码中最相似元素使用相同的 Element Plus 组件 |
